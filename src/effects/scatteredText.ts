...
export class ScatteredText extends BaseEffect {
  ...
  private initParticles(width: number, height: number): void {
    ...
    this.chars = userText || this.config.chars || defaultChars;
  ...
  private updateChars(ctx: UpdateContext): void {
    ...
    const defaultChars = 'つもにはでをがのへとかられ';
    const newChars = currentText || this.config.chars || defaultChars;

    if (newChars !== this.chars) {
      this.chars = newChars;
      for (const p of this.particles) {
        p.textObj.text = this.chars[Math.floor(Math.random() * this.chars.length)];
      }
    }
  }
